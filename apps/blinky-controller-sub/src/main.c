...
#define NODE_NAME "zephyr_blinky_controller"
#define NODE_NAMESPACE ""
#define TOPIC_NAME_MAX_LEN 32
#define TOPIC_NAME "led_control"
#define DOMAIN_ID 0
 ...
 * ============================================================================
 */
static bool gpio_initialize(void);
...
static bool create_entities(void) {
  ...
  RCCHECK(rclc_subscription_init_default(
      &subscriber_int32, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), TOPIC_NAME));

  // create executor
  executor = rclc_executor_get_zero_initialized_executor();
  RCCHECK(rclc_executor_init(&executor, &support.context, 1, &allocator));
  RCCHECK(rclc_executor_add_subscription(&executor, &subscriber_int32,
                                         &msg_int32, &subscription_callback,
                                         ON_NEW_DATA));
